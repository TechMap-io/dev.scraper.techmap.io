...
@Log4j2
class StepstoneJobsScraper extends AWebScraper {
	...
	StepstoneJobsScraper(Integer sourceToScrape) {
	...
	int scrape() {
		...
		for (Element group in groups) {
			def status = AWebScraper.db.loadStatus(sourceID + "-${group.text()}")
	...
	int scrapePageGroup(Element group, Map status) {
		...
		while (nextURL) {
			...
			if (nextURL) {
				...
				AWebScraper.db.saveStatus(status)
			}
		}
	...
	int scrapePageList(Elements pageElements, Map extraData) {
		...
		for (pageElement in pageElements) {
			...
			if (!AWebScraper.db.jobExists(sourceID, idInSource)) {
